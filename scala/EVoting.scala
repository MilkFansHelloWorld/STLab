...
def v0(x : Reject | Ok, c2 : OutChannel[No | Yes], c3 : InChannel[Result], c4 : InChannel[Result]) : V0[x.type, c2.type, c3.type, c4.type] = x match {
  case y : Reject => {
    println(s"-- V received Reject")
    nil
  }
  case y : Ok => {
    ...
    if (x0 == 0) {
      ...
      send(c2, new Yes("")) >> {
        ...
        // println("-- V exits")
        println(s"-- V expecting Result on c3 ($c3)")
        receive(c3) {(x1 : Result) => 
          println(s"-- V received Result on c3 ($c3)")
          println("-- V exits")
          nil
        }
      }
